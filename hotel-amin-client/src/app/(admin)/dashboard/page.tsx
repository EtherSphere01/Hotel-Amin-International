...
const App = () => {
    const [activeTab, setActiveTab] = useState<
        "users" | "rooms" | "food" | "coupons" | "offers"
    >("users");
    ...
    const fetchFood = async () => {
        ...
        try {
            ...
            setErrors((prev) => ({
                ...prev,
                food: "Failed to load menu items",
            }));
        } finally {
    ...
    const checkForNewBookings = async () => {
        try {
            ...
            if (response.ok) {
                ...
                if (
                    previousBookingCount > 0 &&
                    currentCount > previousBookingCount
                ) {
    ...
    const checkForNewReservations = async () => {
        try {
            ...
            if (response.ok) {
                ...
                if (
                    previousReservationCount > 0 &&
                    currentCount > previousReservationCount
                ) {
    ...
    const handleSaveEdit = async (updatedData: any) => {
        ...
        try {
            ...
            if (!response.ok) {
                const errorData = await response.json();
                throw new Error(
                    errorData.message || `Failed to update ${editModalType}`
                );
            }
            ...
            toast.success(
                `${
                    editModalType.charAt(0).toUpperCase() +
                    editModalType.slice(1)
            ...
            const errorMessage =
                error instanceof Error
                    ? error.message
                    : `Failed to update ${editModalType}`;
            toast.error(errorMessage);
        }
    };
    ...
    const handleSaveAdd = async (newData: any) => {
        ...
        try {
            ...
            if (!response.ok) {
                const errorData = await response.json();
                throw new Error(
                    errorData.message || `Failed to create ${addModalType}`
                );
            }
            ...
            toast.success(
                `${
                    addModalType.charAt(0).toUpperCase() + addModalType.slice(1)
    ...
    const transformCouponData = (apiCoupon: any) => ({
        ...
        validFrom: apiCoupon.created_at
            ? new Date(apiCoupon.created_at).toLocaleDateString()
            : "N/A",
        validUntil: apiCoupon.expire_at
    ...
    const fetchOffers = async () => {
        ...
        try {
            ...
            const expiredOffers = offersData.filter(
                (offer: any) => offer.status === "expired"
            ).length;
            setStats((prev) => ({
                ...prev,
                activeOffers,
    ...
    const filteredRooms = displayRooms.filter(
        (room) =>
            room.room_num
                ?.toString()
                .includes(searchTerms.rooms.toLowerCase()) ||
            room.type
                ?.toLowerCase()
                .includes(searchTerms.rooms.toLowerCase()) ||
            room.room_status
    ...
    const filteredFood = displayFood.filter(
        (foodItem) =>
            foodItem.item_name
                ?.toLowerCase()
                .includes(searchTerms.food.toLowerCase()) ||
            foodItem.food_type
                ?.toLowerCase()
                .includes(searchTerms.food.toLowerCase()) ||
    ...
    const filteredCoupons = displayCoupons.filter(
        (coupon) =>
            coupon.coupon_code
                ?.toLowerCase()
                .includes(searchTerms.coupons.toLowerCase()) ||
    ...
    const EditModal = () => {
        ...
        return (
            <div className="fixed inset-0 bg-[#00000047]  bg-opacity-30 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                                ...
                                <>
                                    ...
                                    <div>
                                        ...
                                        >
                                            ...
                                            <option value="waiting_for_clean">
                                                Waiting for Clean
                                            </option>
                                            <option value="needs_service">
                                                Needs Service
                                            </option>
                                        </select>
                                    </div>
    ...
    const AddModal = () => {
        ...
        return (
            <div className="fixed inset-0 bg-[#0000003d] bg-opacity-30 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                <div className="bg-white rounded-2xl shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
                    <div className="p-6">
                        <div className="flex items-center justify-between mb-6">
                            <h2 className="text-xl font-bold text-slate-800">
                                Add New{" "}
                                {addModalType.charAt(0).toUpperCase() +
                                    addModalType.slice(1)}
                            </h2>
                            <button
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex flex-col lg:flex-row">
            ...
            {sidebarOpen && (
                <div
                    className="fixed inset-0 bg-[#00000041] bg-opacity-50 z-40 lg:hidden"
                                            ...
                                            <div className="overflow-x-auto max-w-full">
                                                ...
                                                >
                                                    <thead className="bg-gradient-to-r from-slate-100 to-slate-50">
                                                        <tr>
                                                            <th
                                                                className="px-3 lg:px-6 py-4 text-left text-xs font-bold text-slate-600 uppercase tracking-wider"
                                                                style={{
                                                                    minWidth:
                                                                        "150px",
                                                                }}
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
                                        {loading.rooms ? (
                                            ...
                                            filteredRooms.map((room) => (
                                                <div
                                                    key={room.id}
                                                    className="bg-white rounded-xl shadow-lg border border-slate-200 hover:shadow-xl transition-shadow overflow-hidden"
                                                >
                                ...
                                <div className="space-y-6">
                                    <div className="flex flex-col gap-4">
                                        <div>
                                            ...
                                            <p className="text-slate-600 mt-1">
                                                Manage restaurant menu and food
                                                services
                                            </p>
                                        </div>
                                        <div className="flex flex-col sm:flex-row gap-3">
